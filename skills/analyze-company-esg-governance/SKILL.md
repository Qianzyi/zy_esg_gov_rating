...
8. Put a source table titled "数据来源" after the institutional-investor table. Sources should include annual report, ESG/sustainability report if available, latest governance announcements, company website, and broader web or regulatory checks used. Put only the exact final verification sentence "报告内容已基于公开资料完成二次核验。" after the source table. Do not add a separate verification title or describe the verification process.
...
- Do not omit the "数据来源" table in a default PDF/formal report.
- Do not end the report with any sentence other than the exact final sentence "报告内容已基于公开资料完成二次核验。" after the data source table.
